...
/** The on-demand observe fetch state, lifted to the drawer so the Engine tab
 *  (which triggers it, by being opened) and the Packages tab (which surfaces
 *  per-queue activity) share one fetch instead of each calling the slow
 *  pod-exec endpoint. */
export type ObserveState =
  | { status: 'idle' }
  | { status: 'loading' }
  // `httpStatus` carries the failed request's HTTP status (from `ObserveError`)
  // so the Engine tab can explain itself — e.g. 409 == no durable delivery store
