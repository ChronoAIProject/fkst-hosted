...
/**
 * The two pre-data states of `/workflows`, mirroring `/operations`.
 *
 * Both are deliberately plain. This is not a marketing surface: an
 * unauthenticated visitor gets one sentence about what the route is for and a
 * sign-in button, and a build with no API base gets the one fact that explains
 * why nothing will load.
 */
export function WorkflowsGate({
  error,
  configured,
  onSignIn,
}: {
...
export function WorkflowsUnconfigured() {
  const t = useContent().workflows;
  return (
    <div className="h-full flex items-center justify-center px-6">
