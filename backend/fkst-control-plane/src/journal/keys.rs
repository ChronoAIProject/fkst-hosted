//! Content-derived key derivation (the heart of the redo contract): every
//! raised event gets a stable `idem_key`, and every logical run a
//! content-derived `run_key`, both identical whether produced by the original
//! session or a redo on a different pod. Correctness never depends on
//! timestamps or on the engine's LOCAL `once()` marks / `with_lock` /
//! codex-permits.
...
mod tests {
    ...
    fn idem_key_is_deterministic_lowercase_hex_64() {
        let event = serde_json::json!({"department":"d","source":"s","name":"n","corr":"c"});
    ...
    fn idem_key_changes_with_any_identity_pointer_or_package() {
        let base = serde_json::json!({"department":"d","source":"s","name":"n","corr":"c"});
        let base_key = idem_key("pkg", &base, &pointers());
        for changed in [
            serde_json::json!({"department":"X","source":"s","name":"n","corr":"c"}),
            serde_json::json!({"department":"d","source":"X","name":"n","corr":"c"}),
            serde_json::json!({"department":"d","source":"s","name":"X","corr":"c"}),
            serde_json::json!({"department":"d","source":"s","name":"n","corr":"X"}),
        ] {
    ...
    fn idem_key_all_missing_pointers_uses_a_stable_fallback() {
        let event = serde_json::json!({"weird": true});
        let a = idem_key("pkg", &event, &pointers());
        let b = idem_key("pkg", &event, &pointers());
        assert_eq!(a, b);
        ...
        assert_ne!(
            a,
            idem_key("pkg", &serde_json::json!({"weird": false}), &pointers())
        );
    }
