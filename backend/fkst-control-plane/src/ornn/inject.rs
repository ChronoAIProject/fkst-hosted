...
//!   symlink escapes — mirroring `engine/materialize.rs::safe_join` (defense in
//!   depth against a malicious package planted in the registry).
...
///
/// A self-contained mirror of `engine::materialize::safe_join` (which returns a
/// `RunnerError` in a different domain): absolute paths, backslashes, control
/// characters, `.`/`..`/empty segments, and symlink escapes are all rejected.
/// The symlink-escape guard canonicalizes the deepest EXISTING ancestor of the
/// target and asserts it stays inside the canonicalized `root`, so a symlink
/// planted inside the install dir that points outside is caught.
fn safe_join(root: &Path, rel: &str) -> Result<PathBuf, AppError> {
    if rel.is_empty() {
        return Err(AppError::Unprocessable("empty zip entry path".to_string()));
    }
    ...
    let canonical_root = root.canonicalize().map_err(|error| {
        tracing::error!(error = %error, "failed to canonicalize skill install root");
        AppError::Unavailable("failed to resolve skill install root".to_string())
    })?;
    ...
    let canonical_ancestor = loop {
        match probe.symlink_metadata() {
            Ok(_) => match probe.canonicalize() {
                ...
                Err(error) => {
                    tracing::error!(error = %error, "failed to canonicalize skill path ancestor");
                    return Err(AppError::Unavailable(
                        "failed to resolve skill path".to_string(),
                    ));
                }
            },
...
pub fn install_skill(codex_home: &Path, name: &str, zip_bytes: &[u8]) -> Result<(), AppError> {
    ...
    std::fs::create_dir_all(&skills_root).map_err(|error| {
        ...
        AppError::Unavailable("failed to create skills directory".to_string())
    })?;
    ...
    std::fs::create_dir_all(&skill_dir).map_err(|error| {
        ...
        AppError::Unavailable("failed to create skill directory".to_string())
    })?;
    ...
    let mut archive = zip::ZipArchive::new(reader).map_err(|error| {
        ...
        AppError::Unprocessable("malformed skill package archive".to_string())
    })?;
    ...
    for index in 0..archive.len() {
        let mut entry = archive.by_index(index).map_err(|error| {
            ...
            AppError::Unprocessable("malformed skill package archive entry".to_string())
        })?;
        ...
        if entry.is_dir() {
            ...
            std::fs::create_dir_all(&dir).map_err(|error| {
                ...
                AppError::Unavailable("failed to create skill subdirectory".to_string())
            })?;
            continue;
        }
        ...
        if let Some(parent) = target.parent() {
            std::fs::create_dir_all(parent).map_err(|error| {
                ...
                AppError::Unavailable("failed to create skill parent directory".to_string())
            })?;
        }
        ...
        entry.read_to_end(&mut content).map_err(|error| {
            ...
            AppError::Unprocessable("failed to read skill package entry".to_string())
        })?;
        ...
        std::fs::write(&target, &content).map_err(|error| {
            ...
            AppError::Unavailable("failed to write skill file".to_string())
        })?;
        std::fs::set_permissions(&target, std::fs::Permissions::from_mode(mode)).map_err(
            |error| {
                ...
                AppError::Unavailable("failed to set skill file permissions".to_string())
            },
        )?;
        file_count += 1;
    }
...
pub fn append_instructions(
    ...
    let existing = match std::fs::read_to_string(&agents_path) {
        ...
        Err(error) => {
            tracing::error!(error = %error, "failed to read AGENTS.md");
            return Err(AppError::Unavailable(
                "failed to read codex AGENTS.md".to_string(),
            ));
        }
    };

    let updated = upsert_marker_block(&existing, skillset_name, instructions);
    std::fs::write(&agents_path, updated).map_err(|error| {
        tracing::error!(error = %error, "failed to write AGENTS.md");
        AppError::Unavailable("failed to write codex AGENTS.md".to_string())
    })?;
