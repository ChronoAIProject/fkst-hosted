...
//!   issue, goal prompt, packages, the deterministic session id + log branch).
//!   It holds no credentials, so a `{:?}` of it can never leak a token.
...
//!   token and the static LLM API key). The control plane writes the files; the
//!   pod reads them.
//!
