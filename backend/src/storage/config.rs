...
//! - `FKST_NYXID_CLIENT_ID` — the service-account client id (the single shared
//!   SA — see below).
//! - `FKST_NYXID_CLIENT_SECRET` — the SA's client secret (held in a
//!   [`SecretString`], never logged, redacted in `Debug`).
//!
//! This single service account serves BOTH roles: the control plane authenticates
//! with it for its own storage calls (log downloads), and the reconciler injects
//! the same id/secret into every session pod so the in-pod collector can PUT its
//! log bundle. (A split write-only per-session SA existed historically and may
//! return in a future hardening phase.)
//!
...
pub struct ChronoStorageConfig {
    ...
    /// Held in a [`SecretString`]; never logged and redacted in `Debug`. Also
    /// injected (with the client id) into every session Secret for the in-pod
    /// log uploader — see the module docs.
...
impl ChronoStorageConfig {
    ...
    pub(crate) fn from_vars(vars: &[(String, String)]) -> Result<Option<Self>, AppError> {
        // The retired FKST_STORAGE_WRITER_* pair (the pre-#489 split-SA contract)
        // is silently dropped by envy below; warn so an operator upgrading with it
        // still set learns their dedicated writer SA is no longer what sessions
        // upload with — the single FKST_NYXID_CLIENT_ID SA is.
        for retired in [
            "FKST_STORAGE_WRITER_CLIENT_ID",
            "FKST_STORAGE_WRITER_CLIENT_SECRET",
        ] {
            if vars
                .iter()
                .any(|(k, v)| k == retired && !v.trim().is_empty())
            {
                tracing::warn!(
                    var = retired,
                    "chrono-storage: retired variable is set and ignored; sessions \
                     upload with the single FKST_NYXID_CLIENT_ID service account"
                );
            }
        }
...
mod tests {
    ...
    #[test]
    fn retired_writer_vars_are_ignored() {
        // The historical FKST_STORAGE_WRITER_* pair no longer exists in the
        // contract; a stale deployment still setting it must neither fail nor
        // alter the resolved single-SA config (it only earns a startup warn).
        let mut vars = full();
        vars.push((
        ...
        let config = ChronoStorageConfig::from_vars(&vars)
            .expect("stale writer vars are dropped, not an error")
            .expect("enabled");
        assert_eq!(config.nyxid_client_id, "sa-client");
        assert_eq!(config.nyxid_client_secret.expose_secret(), "sa-secret");
    }
    ...
    fn client_secret_is_redacted_in_debug_output() {
        ...
        assert!(debug.contains("sa-client"), "{debug}");
    }
}
