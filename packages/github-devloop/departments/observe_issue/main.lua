...
local spec = {
  ...
  fanout = { "github-proxy.github_issue_changed", "github-proxy.github_pr_changed" },
  stall_window = "30s",
}
...
local function process_issue_event(event)
  ...
  with_lock(lock_key, function()
    ...
    local function maybe_canonicalize_legacy_pr_open_issue()
      ...
      local label_request = requests_labels.build_state_label_request(issue.repo, issue.number, "awaiting-pr", base_ids.dedup_key({
        "canonicalize",
        "pr-open",
        "label",
        tostring(proposal_id),
...
return saga.department(spec, { done = function() return false end, act = function(event)
  queue.dispatch_consumed_queue("observe_issue", spec, event, {
    ["github-proxy.github_issue_changed"] = process_issue_event,
