...
function M.operator_command_marker(command, outcome, reason)
  if type(command) ~= "table"
    or (command.command ~= "rereview" and command.command ~= "reready" and command.command ~= "reintake" and command.command ~= "reimplement") then
...
function M.build_operator_issue_reimplement_comment_request(repo, issue_number, command, outcome_reason, source_ref)
  local marker = M.operator_command_marker(command, "applied", outcome_reason or "reimplement")
  return M.build_entity_comment_request({
    kind = "issue",
    repo = repo,
    number = issue_number,
    ...
    tostring(command.key),
    "applied",
    tostring(outcome_reason or "reimplement"),
  }), source_ref)
end
