...
return {
  ...
  test_implement_ready_runs_codex_in_worktree_and_marks_implementing = function()
    ...
    t.eq(m_facts.implementing_fact({ state_raise.payload.body }, event.proposal_id, event.dedup_key), nil)
    ...
    local fact = m_facts.implementing_fact({ comment_raise.payload.body }, event.proposal_id, event.dedup_key)
