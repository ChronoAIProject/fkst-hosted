...
return {
  ...
  test_git_methods_build_argv = function()
    ...
    handle.commit_message_file("/tmp/wt", "/tmp/message.txt", 39)
    handle.worktree_add_detached("/tmp/wt", "eeee5555", 40)
    handle.worktree_remove("/tmp/wt", 41)
    ...
    assert_argv_equal(calls[19].argv, { "git", "-C", "/tmp/wt", "commit", "-F", "/tmp/message.txt" }, "commit_message_file")
    assert_argv_equal(calls[20].argv, { "git", "worktree", "add", "--detach", "/tmp/wt", "eeee5555" }, "worktree_add_detached")
    assert_argv_equal(calls[21].argv, { "git", "worktree", "remove", "--force", "/tmp/wt" }, "worktree_remove")
    for index, call in ipairs(calls) do
