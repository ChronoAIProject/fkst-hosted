...
return {
  ...
  test_fix_blocks_when_post_codex_unmerged_index_remains = function()
    local event = fixing({ gate_baseline_sha = "abc123", gate_failure_excerpt = "own-ci-red" })
    local branch = devloop_base.implement_branch("owner/repo", "42", event.version)
    local reject_comment = "github-devloop merge gate failed: own-ci-red"
      .. "\n" .. core.state_marker(event.proposal_id, "fixing", event.version)
    ...
    mock_existing_fix_worktree(branch, "def456", nil, {
      ...
      stderr = "CONFLICT (content): Merge conflict in packages/github-devloop/core.lua\n",
      unmerged_stdout = "100644 abc123 1\tpackages/github-devloop/core.lua\n",
      post_codex_unmerged_stdout = "100644 def456 2\tpackages/github-devloop/core.lua\n",
    })
    mock_write_env("1")
    mock_implement_codex(0, "conflict still unresolved")

    local result = run_fix(event, opts("fix-post-codex-unmerged-blocks", { FKST_GITHUB_WRITE = "1" }))
    t.eq(result.exit_code, 1)
    ...
    t.eq(count_calls("diff --cached --check"), 0)
    t.eq(count_calls("commit -m"), 0)
    t.eq(count_calls("git push origin"), 0)
  end,
  ...
  test_fix_clean_ahead_reuse_blocks_on_range_diff_check = function()
    ...
    t.mock_command("git -C " .. worktree .. " diff --check " .. event.reviewed_head_sha .. "..feedface", {
      stdout = "IMPLEMENT_REPORT.md:8: leftover conflict marker\n",
      stderr = "range diff check details\n",
      exit_code = 2,
    })
    mock_write_env("1")
    ...
    t.eq(result.exit_code, 1)
    local saw_range_diff_check = false
    for _, call in ipairs(t.command_calls()) do
      if gh_argv.argv_contains(call, { "git", "-C", worktree, "diff", "--check", event.reviewed_head_sha .. "..feedface" }) then
        saw_range_diff_check = true
      end
    end
    t.is_true(saw_range_diff_check)
    t.is_true(tostring(result.error or ""):find("range-diff-check-failed", 1, true) ~= nil)
    t.is_true(tostring(result.error or ""):find("base=def456", 1, true) ~= nil)
    t.is_true(tostring(result.error or ""):find("head=feedface", 1, true) ~= nil)
    t.is_true(tostring(result.error or ""):find("stdout=IMPLEMENT_REPORT.md:8: leftover conflict marker\\n", 1, true) ~= nil)
    t.is_true(tostring(result.error or ""):find("stderr=range diff check details\\n", 1, true) ~= nil)
    t.eq(count_calls("add -A"), 0)
    t.eq(count_calls("commit -m"), 0)
    t.eq(count_calls("git push origin"), 0)
  end,
