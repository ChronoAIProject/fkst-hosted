...
local tests = {
  ...
  test_generated_fact_raises_one_issue_create_request_with_lineage = function()
    ...
    t.eq(#creates[1].payload.labels, 1)
    t.eq(creates[1].payload.labels[1], "fkst-dev")
    t.is_true(creates[1].payload.labels[1] ~= "fkst-workflow")
    t.is_true(creates[1].payload.labels[1]:find("fkst-dev:", 1, true) == nil)
