...
local function build_generated_prompt(ctx, slot, predecessor_result_ref, content_fetch_ref)
  ...
  return table.concat({
    ...
    "The generated issue body must be independently self-contained for a downstream worker that cannot read the origin issue or predecessor result.",
    "Copy every exact external contract or wire grammar the worker needs into the generated body, including required literal examples, field names, ordering or escaping rules, and acceptance details.",
    "Never refer vaguely to a contract in an absent origin or predecessor, and never require the downstream worker to fetch either source.",
