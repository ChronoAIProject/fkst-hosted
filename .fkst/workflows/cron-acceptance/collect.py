...
The ledger read is what makes the second run of a cron schedule provably
different from the first, which is the property #5846 asks this step to
demonstrate. The window slides by one id per run and overlaps the previous run
by one, so a single run shows BOTH behaviours at once: an already-published id
suppressed, and genuinely new ids carried forward. A window with no overlap
would prove only that ids changed, not that the ledger was consulted.
