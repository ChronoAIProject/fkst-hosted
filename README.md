...
`packages/github-devloop/` 是组合 `github-proxy` + `consensus` 的 composed 包。它覆盖 issue design consensus 与 stuck meta-escalation：带 `fkst-dev:enabled` 且没有 `fkst-dev:thinking|ready|blocked|stuck` 的 GitHub issue 被回源读取 body，映射成 `consensus.proposal`，并请求 `github-proxy` 加 `fkst-dev:thinking`；当 `consensus.consensus_reached` 返回 `approve` / `reject` 时，请求把 issue 改成 `fkst-dev:ready` / `fkst-dev:blocked`，并发一条带 `fkst:github-devloop:result:v1` HTML marker 的评论作为外部 durable 记录。

当 `consensus.consensus_unresolved` 到达时，`departments/loop` 回源读取当前 issue label、body 与评论 marker：issue 仍处于 `fkst-dev:thinking` 且未达到预算时，重建并重新 raise `consensus.proposal`，同时请求写入 `fkst:github-devloop:loop:v1` marker；达到预算时请求设置 `fkst-dev:stuck`、移除 `fkst-dev:thinking`，写入 `fkst:github-devloop:stuck:v1` marker，并产生包内裸名 `devloop_stuck` 事件（schema `github-devloop.stuck.v1`）。

`departments/meta` 消费 `devloop_stuck`，回源确认 issue 仍有 `fkst-dev:stuck` 且没有当前版本的 `fkst:github-devloop:meta:v1` marker 后，运行一次 meta `codex exec`，只接受严格相邻的 `⟦FKST:ACTION⟧ implement|split|block` 与 `⟦FKST:REASON⟧ ...` 输出。`implement` 请求转到 `fkst-dev:ready`，`split` / `block` 请求转到 `fkst-dev:blocked`；`split` 只在 marker 评论里记录建议，不自动创建 GitHub issue。loop 与 meta 的事实源都来自 GitHub label / HTML marker，不用 `<RT>` 或 cache；所有 label/comment 写入都经 `github-proxy` 的 dry-run-by-default 出站队列。`github-devloop/composed.deps` 声明它需要把 `github-proxy` 与 `consensus` 一起加载做组合 conformance。
