...
Phase 4 里，`open_pr` 在 `implementing` + `fkst-dev:pr-authorized` + `FKST_GITHUB_WRITE=1` 时经 `github-proxy.github_pr_open_request` 打开 PR 并写 issue/PR linkage marker；`observe_pr` 通过 PR 上可信 `fkst:github-devloop:pr-origin:v1` backpointer 把 issue 从 `pr-open` 推进到 `reviewing`，同时产生包内 `devloop_reviewing` 事件。Phase 5a 里，`review_pr` 消费 `devloop_reviewing`，回源确认 issue canonical state 仍是同版本 `reviewing` 后，用独立预算保留 bounded `gh pr diff <n>`，再附加较小的 bounded issue context，构造 source-agnostic `consensus.proposal`（`github-devloop/pr-review/.../<head_sha>`）；diff 和 issue 文本都作为 untrusted data block 中和 marker / sentinel。`review_result` 只消费该 pr-review proposal 的 `consensus.consensus_reached`：它重新读取 PR trusted backpointer 和当前 head，要求当前 head 仍等于 review proposal 绑定的 `head_sha`，再以 issue state marker CAS 把 `reviewing` 推进到 `merge-ready`（`approve`）或 `fixing`（`reject`），写 issue state marker、`fkst:github-devloop:review-result:v1` marker 与 set-exclusive label；issue canonical state marker version 仍用 issue implementation version，review consensus dedup 只用于 review-result 幂等 marker。`consensus.consensus_unresolved` 对 pr-review proposal 目前没有专门处理器，issue 保持 `reviewing`；fix loop 与 review meta-escalation 留给 Phase 5b。Phase 5a 不 push、不 merge。
