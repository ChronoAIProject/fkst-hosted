...
### Installation lifecycle: persistence, webhook, and uninstall (#108)

The App layer no longer resolves installations purely reactively and in memory. A durable, observed lifecycle backs it:

**Persistence (`github_installations` collection).** Each installation is one document keyed by the GitHub installation id (`_id`, a `u64`). It records the account the App is installed on — which may be a personal **user** account or an **organization** (`account_type: User | Organization`; do **not** assume the installer equals the account, nor a user account) — the `repository_selection` (`all` vs `selected`), the covered `repos` (lowercase `owner/name` full names; the sentinel `"*"` for an `all` install), a `suspended` flag, and `updated_at`. Two indexes back resolution: `repos` (membership → which install covers a repo) and `account_login` (an `all` install by owner). The Mongo-backed `MongoInstallationStore` is the production implementation of the `InstallationStore` seam injected into the token service, so the service depends only on the abstraction (an in-memory fake backs the unit tests).

**Resolve-from-persistence.** `token_for_repo` / installation resolution now checks, in order: (1) the in-memory installation cache (1h TTL), (2) the persistence store, (3) the on-demand `GET /repos/{owner}/{repo}/installation` GitHub probe. A persistence hit means **no live GitHub probe** — so a cold pod after a restart resolves from the DB rather than re-probing, and a suspended/removed install is reflected immediately. After an on-demand probe (a persistence miss) the resolved mapping is persisted (non-destructively, via `$addToSet`) so the next resolve is a store hit. A store read/write failure is logged and treated as a fall-through, never a hard dependency of minting.

**Webhook endpoint (`POST /api/v1/github/app/webhook`).** UNAUTHENTICATED but signature-verified, mounted **outside** the `/api/v1` auth nest (like `/health`) and only when `FKST_GITHUB_APP_WEBHOOK_SECRET` is set. The handler:
...
2. Parses `X-GitHub-Event` and dispatches: `installation` (`created`/`deleted`/`suspend`/`unsuspend`) and `installation_repositories` (`added`/`removed`) upsert/remove the persisted record. The handler is **idempotent** (GitHub redelivers) and returns `2xx` quickly; an internal processing failure logs and answers `202` rather than triggering a redelivery storm.

**Uninstall → caches → sessions.** On an `installation deleted` / `suspend` / `repository removed` event, the affected repos are evicted from BOTH in-memory caches and the persisted record (so the next mint correctly 404s), and any **active session** targeting an affected repo is transitioned to `Failed` with a clear reason (e.g. `GitHub App was uninstalled from or lost access to <owner/repo>`) — rather than the session breaking later on a silent 401. (The in-run periodic refresh's `InstallationGone` backstop, below, still covers the case where no webhook is configured.)
...
| `FKST_GITHUB_APP_WEBHOOK_SECRET` | No (recommended) | HMAC secret for `POST /api/v1/github/app/webhook` (issue #108). When set the webhook route is mounted and signature-verified; when unset the route is not mounted and installation resolution degrades to on-demand (a warning is logged). Held in a `SecretString`; never logged. |
