...
local function act(event)
  ...
  local written, repo = core.write_with_outbound_log(payload, {
    ...
    view_comments = function(github, target_repo, number, timeout)
      return github.issue_comments(target_repo, number, timeout)
    end,
    comment_create = function(github, target_repo, number, body_file, timeout)
      return github.issue_comment_create(target_repo, number, body_file, timeout)
    end,
...
M.done = done
M.act = act
M.wrap = core.wrap_pipeline_failure
M.name = "github_comment"
